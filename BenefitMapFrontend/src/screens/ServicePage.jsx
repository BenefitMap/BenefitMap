    ...
    const handleAddToCalendar = useCallback(
        async service => {
            ...
            try {
                ...
                if (!existingRes.ok) {
                    console.warn('캘린더 목록 조회 실패:', existingRes.status);
                }

                const existingItems = existingRes.ok ? await existingRes.json() : [];
                const alreadyExists = Array.isArray(existingItems)
                    ? existingItems.some(item => item.welfareId === service.id)
                    : false;
