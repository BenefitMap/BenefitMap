...
export const useAuth = () => {
   ...
   *     if (!ok) return; // 여기서 return하면 뒤 로직 중단
   *     // 여기서부터 실제 페이지 데이터 로드 가능
   *   })();
   * }, []);
   */
  ...
  useEffect(() => {
    ...
    //    (예: 토큰 만료돼서 fetchUserInfo()가 나중에 user_info를 지우는 경우도 있음)
  ...
  /**
   * 외부(예: 로그인 직후, 로그아웃 직후 등)에서 강제로 다시 동기화시키고 싶을 때 쓸 수 있는 helper.
   * 예: refreshAuth() 호출하면 localStorage -> state 다시 반영.
   */
  ...
  return {
    ...
    // 로그아웃 자체는 utils/auth.logout(navigate) 쓰면 됨
  };
};
