...
public class OnboardingService {
    ...
    public void save(Long userId, OnboardingRequest req) {
        ...
        var interest = interestCodes.isEmpty() ? List.<InterestTag>of() : interestRepo.findByCodeIn(interestCodes);
        ...
        house.forEach(t -> userHouseholdRepo.save(
                UserHouseholdTag.builder().id(new UserHouseholdTagId(userId, t.getId())).build()
        ));

        userInterestRepo.deleteByIdUserId(userId);
        interest.forEach(t -> userInterestRepo.save(
